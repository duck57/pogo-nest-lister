...
# Probably can share more code with sort.py methods, but they're slightly different
...
@click.command()
@click.option('-d', '--date', default=str(datetime.datetime.today().date()), prompt="Date of nest shift", help="Date when the nest shift occured, can be absolute (YYYY-MM-DD) or relative (w+2)")
@click.option('-c', '--city', '-f', prompt="City to rotate", help="The chosen city will be the one to have a fresh copy of its nest list created in its folder in hist-list/")
